...
public class OrderController {
    ...
    public ResponseEntity<ApiResponse<OrderResponse>> createOrder(@Valid @RequestBody OrderRequest request) {
        try {
            log.info("Received order request for user: {}, product: {}",
                    request.getUsername(), request.getProductName());
    ...
    @GetMapping("/get")
    public ResponseEntity<ApiResponse<List<OrderResponse>>> getOrders(@RequestParam String username) {
        try {
            List<OrderResponse> orders;

            if (username != null && !username.trim().isEmpty()) {
                log.info("Received request to get orders for username: {}", username);
